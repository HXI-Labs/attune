...
def test_selection_excludes_all_held_out_ids_and_is_balanced() -> None:
    ...
    assert Counter((row["partition"], row["probe_label"]) for row in rows) == {
        ...
        ("validation", label): 1
        for label in PREPARE.SOURCE_TO_PROBE_LABEL.values()
    }
...
def test_committed_probe_manifest_is_clip_disjoint_and_licence_clean() -> None:
    ...
    rows = PREPARE.json.loads(
        ...
        + ",".join(
            line
            for line in (
                repository / "data/manifests/fsd50k-frozen-probe.jsonl"
            ).read_text(encoding="utf-8").splitlines()
            if line.strip()
        )
        + "]"
    )
    held_out = PREPARE.inspection_ids(
        repository / "data/manifests/licence-clean-inspection.jsonl"
    )

    assert len(rows) == 320
    assert not (
        {row["clip_id"].removeprefix("fsd50k-") for row in rows} & held_out
    )
    assert {
        row["licence"]["clip_identifier"] for row in rows
    } <= {"CC0-1.0", "CC-BY-3.0"}
    assert all(
        len(set(row["labels"]) & set(PREPARE.SOURCE_TO_PROBE_LABEL)) == 1
        for row in rows
    )
