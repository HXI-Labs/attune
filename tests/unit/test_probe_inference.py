...
def test_frozen_linear_probe_emits_validation_calibrated_confidence(
    tmp_path: Path,
) -> None:
    labels = ["shout", "whisper", "sob", "scream"]
    feature_count = FrozenSenseVoiceEncoder.output_size
    ...
    with torch.no_grad():
        linear.weight.zero_()
        linear.bias.copy_(torch.tensor([4.0, 0.0, 0.0, 0.0, -4.0]))
    checkpoint = tmp_path / "head.pt"
    torch.save(
        {
            "head_state_dict": linear.state_dict(),
            ...
            "labels": labels,
            "embedding": SENSEVOICE_EMBEDDING,
            "abstention": {"method": "none_logit", "threshold": 0.0},
        },
        checkpoint,
    )
    provider = SimpleNamespace(
    ...
    raw = prediction.diagnostics["uncalibrated_class_probabilities"]["shout"]
    assert prediction.annotations[0].confidence < raw
    assert prediction.diagnostics["calibration_fitted_on"] == "validation"
    assert prediction.diagnostics["abstention_score_uses_uncalibrated_margin"] is True
