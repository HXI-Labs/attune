...
def test_audio_contract() -> None:
    assert validate_audio_contract(
        sample_rate_hz=16_000, channels=1, duration_seconds=3.0
    ).valid
...
def test_trusted_channel_never_concatenates_metadata(example_output) -> None:
    ...
    assert set(package) == {"user_text", "paralinguistic_metadata"}
    assert package["user_text"] == "I said leave me alone"
    assert "transcript" not in package["paralinguistic_metadata"]
    assert "interpretation_warning" not in package["user_text"]
